...
describe('BinanceProvider', () => {
    ...
    it('should format symbol correctly for subscription', async () => {
        ...
        expect(mockInstance.send).toHaveBeenCalledWith(expect.stringContaining('"params":["btcusdt@kline_1m"]'));
    });
    ...
    it('should handle incoming kline messages', async () => {
        ...
        expect(candle.close).toBe(50100.00);
    ...
    it('should fetch historical candles', async () => {
        ...
        const candles = await provider.getHistoricalCandles('BTC/USDT', '1m', 1638316800000, 1638320400000);

        expect(candles.length).toBe(1);
        expect(candles[0].symbol).toBe('BTC/USDT');
        expect(candles[0].close).toBe(50100.00);
